...
' 
' *****************  Version 2  *****************
' User: Blake Henry  Date: 1/12/05    Time: 6:03p
' Updated in $/Projects/Bitwise/QuickUSB/Software/QuickUsbControl
' Updated to include the latest version of the QuickUSB UserControl
'
' *****************  Version 1  *****************
' User: Blake Henry  Date: 11/20/04   Time: 5:27p
' Created in $/Projects/Oluma/Software
' Oluma DTS Diagnostics
'
' *****************  Version 1  *****************
...
'      If Not IsMissing(status) Then
'         status.SimpleText = "FPGA configuration failed"
'      End If
      UploadFPGA = False
   Else
'      If Not IsMissing(status) Then
'         status.SimpleText = "FPGA was successfully configured"
'      End If
      UploadFPGA = True
   End If
   Exit Function
...
Public Function ReadSPI(ByVal regAddr As Byte, ByRef Value As Byte) As Variant
   ...
   ' Check the transaction result
   If (result = 0) Then
      MsgBox "Cannot set read SCCB", vbCritical + vbOKOnly, Name
      Exit Function
   End If
End Function
...
Public Function WriteSPI(ByVal regAddr As Byte, ByVal Value As Byte) As Variant
   ...
   ' Check the transaction result
   If (result = 0) Then
      MsgBox "Cannot set write SCCB", vbCritical + vbOKOnly, Name
      Exit Function
   End If
End Function
