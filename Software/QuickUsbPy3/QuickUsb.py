...
PPQBYTE = ctypes.POINTER(ctypes.POINTER(QBYTE))
...
class DataFlags(int):
    """QUickUSB Data Flags Enumeration"""
    OutOfOrder = 1
...
#QRETURN QuickUsbGetLastDriverError(QULONG *error);
try:
    QuickUsbLibrary.QuickUsbGetLastDriverError.argtypes = [PQULONG]
    QuickUsbLibrary.QuickUsbGetLastDriverError.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbGetLastDriverError = None
...
#QRETURN QuickUsbLibrary.QuickUsbReadDataEx(QHANDLE hDevice, QBYTE *data, QULONG *length, QULONG flags);
try:
    QuickUsbLibrary.QuickUsbReadDataEx.argtypes = [QHANDLE, PQBYTE, PQULONG, QULONG]
    QuickUsbLibrary.QuickUsbReadDataEx.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbReadDataEx = None
...
#QRETURN QuickUsbLibrary.QuickUsbWriteDataEx(QHANDLE hDevice, QBYTE *data, QULONG *length, QULONG flags);
try:
    QuickUsbLibrary.QuickUsbWriteDataEx.argtypes = [QHANDLE, PQBYTE, PQULONG, QULONG]
    QuickUsbLibrary.QuickUsbWriteDataEx.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbWriteDataEx = None
...
#QUSBLIB QRETURN QuickUsbAllocateDataBuffer(PQBYTE *buffer, QULONG bytes);
try:
    QuickUsbLibrary.QuickUsbAllocateDataBuffer.argtypes = [PPQBYTE, QULONG]
    QuickUsbLibrary.QuickUsbAllocateDataBuffer.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbAllocateDataBuffer = None

#QUSBLIB QRETURN QuickUsbFreeDataBuffer(PQBYTE *buffer);
try:
    QuickUsbLibrary.QuickUsbFreeDataBuffer.argtypes = [PPQBYTE]
    QuickUsbLibrary.QuickUsbFreeDataBuffer.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbFreeDataBuffer = None
...
#QUSBLIB QRETURN QuickUsbBulkAbort(QHANDLE hDevice, PQBULKSTREAM bulkStream);
try:
    QuickUsbLibrary.QuickUsbBulkAbort.argtypes = [QHANDLE, PQBULKSTREAM]
    QuickUsbLibrary.QuickUsbBulkAbort.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbBulkAbort = None

#QUSBLIB QRETURN QuickUsbSetNumAsyncThreads(QHANDLE hDevice, QWORD numThreads, QWORD concurrency);
try:
    QuickUsbLibrary.QuickUsbSetNumAsyncThreads.argtypes = [QHANDLE, QWORD, QWORD]
    QuickUsbLibrary.QuickUsbSetNumAsyncThreads.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbSetNumAsyncThreads = None

#QUSBLIB QRETURN QuickUsbGetNumAsyncThreads(QHANDLE hDevice, PQWORD numThreads, PQWORD concurrency);
try:
    QuickUsbLibrary.QuickUsbGetNumAsyncThreads.argtypes = [QHANDLE, PQWORD, PQWORD]
    QuickUsbLibrary.QuickUsbGetNumAsyncThreads.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbGetNumAsyncThreads = None
...
try:
    QuickUsbLibrary.QuickUsbGetStatistic.argtypes = [QHANDLE, QULONG, QULONG, PQFLOAT, QULONG]
    QuickUsbLibrary.QuickUsbGetStatistic.restype = QRESULT
except AttributeError:
    QuickUsbLibrary.QuickUsbGetStatistic = None
...
def QuickUsbLog(qusb, func, result, **kwargs):
    ...
    if QuickUsb.QuickUsbLogFunc:
        if result:
            QuickUsb.QuickUsbLogFunc('%s() exited successfully' % func.__name__)
        else:
            err = qusb.LastError()
            QuickUsb.QuickUsbLogFunc('%s() exited with error #%i (%s)' % (func.__name__, int(err), str(err)))
...
class QuickUsb(object):
    ...
    def FindModules(cls):
        ...
        if not result:
            ...
            lastDriverError = QuickUsb.GetLastDriverError()
            return (result, [])

        bytenames = nameList.value.split(b'\x00')
        strnames = []
        for bname in bytenames:
            strnames.append(bname.decode())
        if len(strnames) == 0:
            return (result, [])
        return (result, strnames)
    ...
    def GetLastDriverError(cls):
        """
        Static class method to retrieve the last QuickUSB Driver error code.
        
        Designed to only be used with static class methods such as
        FindModules(), GetDriverVersion(), and GetDllVersion().  Otherwise,
        use QuickUsb.LastDriverError().
        ...
        Returns:
            ...
            function call.
        """
        lastDriverError = QULONG()
        QuickUsbLibrary.QuickUsbGetLastError(ctypes.byref(lastDriverError))
        return lastDriverError.value
    GetLastDriverError = classmethod(GetLastDriverError)
    ...
    def AllocateDataBuffer(cls, bytes):
        """
        Allocate a data buffer 'bytes' long that will work on for asynchronous
        and streaming data requests on all platforms.
        
        Keyword Arguments:
            bytes   -- The number of bytes to allocate
            
        Returns:
            A tuple of the form (ok, buf).
                ok      -- Indicates that the function call succeeded or failed
                buf     -- A c_types data buffer for use with the asynchronous
                           and streaming data APIs.
        """
        #QRETURN <= [PPQBYTE, QULONG]
        buf = PQBYTE()
        result = QuickUsbLibrary.QuickUsbAllocateDataBuffer(ctypes.byref(buf), QULONG(bytes))

        # Log the result
        #QuickUsbLog(self, QuickUsb.AllocateDataBuffer, result)

        return (result, buf)
    AllocateDataBuffer = classmethod(AllocateDataBuffer)



    def FreeDataBuffer(cls, buf):
        """
        Free a data buffer previously allocated with AllocateDataBuffer()
        ...
        """
        #QRETURN <= [PPQBYTE]
        result = QuickUsbLibrary.QuickUsbFreeDataBuffer(ctypes.byref(buf))

        # Log the result
        #QuickUsbLog(self, QuickUsb.FreeDataBuffer, result)

        return (result,)
    FreeDataBuffer = classmethod(FreeDataBuffer)
    ...
    def SetThrowExceptions(self, value):
        ...
        for (k, v) in QuickUsbLibrary.__dict__.items():
    ...
    def __init__(self, devName=None):
        ...
        self.lastDriverError = 0
        self._CompletionRoutine = {}
    ...
    def _OpenPolitely(self):
        ...
        if self.localOpen == 1:
            ...
            if not ok:
                ...
                self.lastDriverError = self.GetLastDriverError()
    ...
    def LastDriverError(self):
        """
        Retrieve the last QuickUSB Driver error code.
    ...
    def Open(self, devname=""):
        ...
        if (devname != ""):
            self.devName = devname
        result = QuickUsbLibrary.QuickUsbOpen(ctypes.byref(self.hDevice), ctypes.create_string_buffer(self.devName.encode()))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
        else:
    ...
    def GetStringDescriptor(self, stringIndex):
        ...
        string = string[:].split(b'\0')[0].decode()
    ...
    def ReadDataEx(self, c_array, bytes, flags):
        """
        Read a block of data bytes from the high-speed parallel port using the
        QuickUSB module.
        ...
        Keyword Arguments:
            c_array -- A ctypes-style byte array that is filled with the read
                       ...
                       QuickUsb.CreateByteBuffer().
            bytes   -- The number of bytes to read
            flags   -- Additional flags.  Specify DataFlags.OutOfOrder to
                       issue the read request immediately without waiting for
                       any previously issued asynchronous reads or writes to
                       complete.  Any in-flight asynchronous requests will 
                       first complete before this request will be issued.
        ...
        #QRETURN <= [QHANDLE, PQBYTE, PQULONG]
        result = QuickUsbLibrary.QuickUsbReadDataEx(self.hDevice, c_array, ctypes.byref(c_bytes), QULONG(flags))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()

        # Log the result
        #QuickUsbLog(self, QuickUsb.ReadDataEx, result)
    ...
    def ReadDataWordsEx(self, c_array, words, flags):
        """
        Read a block of data words from the high-speed parallel port using the
        QuickUSB module.
        ...
        #QRETURN <= [QHANDLE, PQBYTE, PQULONG]
        c_word_array = ctypes.cast(c_array, ctypes.POINTER(QBYTE))
        result = QuickUsbLibrary.QuickUsbReadDataEx(self.hDevice, c_word_array, ctypes.byref(c_bytes), QULONG(flags))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()

        # Log the result
        #QuickUsbLog(self, QuickUsb.ReadDataWordsEx, result)
    ...
    def WriteDataEx(self, c_data, bytes, flags):
        """
        Write a block of data bytes to the high-speed parallel port using the 
        QuickUSB module.
        ...
        Keyword Arguments:
            ...
            bytes   -- The number of bytes to write
            flags   -- Additional flags.  Specify DataFlags.OutOfOrder to
                       issue the write request immediately without waiting for
                       any previously issued asynchronous reads or writes to
                       complete.  Any in-flight asynchronous requests will 
                       first complete before this request will be issued.
        ...
        #QRETURN <= [QHANDLE, PQBYTE, QULONG]
        result = QuickUsbLibrary.QuickUsbWriteDataEx(self.hDevice, c_data, QULONG(bytes), QULONG(flags))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()

        # Log the result
        #QuickUsbLog(self, QuickUsb.WriteDataEx, result)
    ...
    def WriteDataWordsEx(self, c_array, words, flags):
        """
        Write a block of data words to the high-speed parallel port using the
        QuickUSB module.
        ...
        #QRETURN <= [QHANDLE, PQBYTE, QULONG]
        c_byte_array = ctypes.cast(c_array, ctypes.POINTER(QBYTE))
        result = QuickUsbLibrary.QuickUsbWriteDataEx(self.hDevice, c_byte_array, c_bytes, QULONG(flags))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()

        # Log the result
        #QuickUsbLog(self, QuickUsb.WriteDataWordsEx, result)
    ...
    def ReadBulkDataAsync(self, c_data, bytes, cRoutine, tag):
        ...
        if cRoutine not in self._CompletionRoutine:
            self._CompletionRoutine[cRoutine] = PQBULKSTREAM_COMPLETION_ROUTINE(cRoutine)

        #QRETURN <= [QHANDLE, PQBYTE, QULONG, PQBULKSTREAM, PQBULKSTREAM_COMPLETION_ROUTINE, PQVOID]
        c_bulkStream = QBULKSTREAM()
        result = QuickUsbLibrary.QuickUsbReadBulkDataAsync(self.hDevice, c_data, QULONG(bytes), ctypes.pointer(c_bulkStream), self._CompletionRoutine[cRoutine], PQVOID(tag))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
            self._ClosePolitely()
    ...
    def WriteBulkDataAsync(self, c_data, bytes, cRoutine, tag):
        ...
        #QRETURN <= [QHANDLE, PQBYTE, QULONG, PQBULKSTREAM, PQBULKSTREAM_COMPLETION_ROUTINE, PQVOID]
        c_bulkStream = QBULKSTREAM()
        result = QuickUsbLibrary.QuickUsbWriteBulkDataAsync(self.hDevice, c_data, QULONG(bytes), ctypes.pointer(c_bulkStream), self._CompletionRoutine[cRoutine], PQVOID(tag))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
            self._ClosePolitely()
    ...
    def BulkWait(self, bulkStream, immediate):
        ...
        #QRETURN <= [QHANDLE, PQBULKSTREAM, QBYTE]
        result = QuickUsbLibrary.QuickUsbBulkWait(self.hDevice, bulkStream, QBYTE(immediate))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
    ...
    def BulkAbort(self, bulkStream):
        ...
        Keyword Arguments:
            bulkStream -- A QBULKSTREAM structure that is filled out with
                          ...
                          returned from ReadBulkDataAsync or
                          WriteBulkDataAsync after an asynchrnous request has
                          been successfully issued.  Pass zero (0) to abort 
                          all pending asynchronous IO requests.
        ...
        result = QuickUsbLibrary.QuickUsbBulkAbort(self.hDevice, bulkStream)
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()

        # Log the result
        #QuickUsbLog(self, QuickUsb.BulkAbort, result)
    ...
    def SetNumAsyncThreads(self, numThreads, concurrency):
        """
        Set the number of threads and the thread concurrency for the 
        Asynchronous Data API.

        Keyword Arguments:
            numThreads  -- A word indicating the number of threads to allocate
                           for the Asynchronous API.  If numThreads is zero, 
                           no threads are created and the Asynchronous API 
                           will be single-threaded.  If numThreads is greater 
                           than zero, the Asynchronous API is multithreaded--
            ...
            concurrency -- A word indicating the number of threads that are 
                           allowed to execute completion routines 
                           simultaneously.  Specify zero if numThreads is 
                           zero.  When multithreading, the number of threads 
                           allowed to execute completion routines concurrently
                           will be limited by the value of 'concurrency', 
                           which must be at least one.  The recommended 
                           concurrency to maximize multithreading performance 
                           is equal to the total number of processors/cores in
                           the system.
        ...
        #QRETURN <= [QHANDLE, QWORD, QWORD]
        result = QuickUsbLibrary.QuickUsbSetNumAsyncThreads(self.hDevice, numThreads, concurrency)
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()

        # Log the result
        #QuickUsbLog(self, QuickUsb.SetNumAsyncThreads, result)
    ...
    def GetNumAsyncThreads(self):
        ...
        Returns:
            A tuple of the form (ok, numThreads, concurrency).
                ok          -- Indicates that the function call succeeded or 
                               failed
                numThreads  -- A word indicating the number of threads to 
                               allocatefor the Asynchronous API.  If 
                               numThreads is zero, no threads are created and 
                               the Asynchronous API will be single-threaded.  
                               If numThreads is greater than zero, the 
                               Asynchronous API is multithreaded--the 
                               indicated number of threads will be created 
                               and dedicated to asynchronously processing 
                               completion routines.  When multithreading, the 
                               recommended number of threads to maximize 
                               multithreading performance is equal to the 
                               number of processors/cores in the system 
                               times two.
                concurrency -- A word indicating the number of threads that 
                               areallowed to execute completion routines 
                               simultaneously.  Specify zero if numThreads is 
                               zero.  When multithreading, the number of 
                               threads allowed to execute completion routines 
                               concurrently will be limited by the value of 
                               'concurrency', which must be at least one.  The
                               recommended concurrency to maximize 
                               multithreading performance is equal to the 
                               total number of processors/cores in the system.
        """
        # Don't politely open as we must already be open for this call
        ...
        numThreads = QWORD()
        concurrency = QWORD()
        result = QuickUsbLibrary.QuickUsbGetNumAsyncThreads(self.hDevice, ctypes.byref(numThreads), ctypes.byref(concurrency))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()

        # Log the result
        #QuickUsbLog(self, QuickUsb.GetNumAsyncThreads, result)

        # Don't politely close module because we're still performing async requests
        return (result, numThreads.value, concurrency.value)
    ...
    def ReadBulkDataStartStream(self, c_dataArr, numBuffers, bufferSize, cRoutine, tag, numThreads, concurrency):
        ...
        streamID = QBYTE()
        result = QuickUsbLibrary.QuickUsbReadBulkDataStartStream(self.hDevice, c_dataArr, QULONG(numBuffers), QULONG(bufferSize), self._CompletionRoutine[cRoutine], PQVOID(tag), ctypes.byref(streamID), QWORD(numThreads), QWORD(concurrency))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
    ...
    def WriteBulkDataStartStream(self, c_dataArr, numBuffers, bufferSize, cRoutine, tag, numThreads, concurrency):
        ...
        result = QuickUsbLibrary.QuickUsbWriteBulkDataStartStream(self.hDevice, c_dataArr, QULONG(numBuffers), QULONG(bufferSize), self._CompletionRoutine[cRoutine], PQVOID(tag), ctypes.byref(streamID), QWORD(numThreads), QWORD(concurrency))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
    ...
    def ReadBulkDataStartStreamToFile(self, path, numBuffers, bufferSize, maxTransfers, flags):
        ...
        Keyword Arguments:
            ...
            flags        -- (Optional) Additional flags.  Pass 
                            StreamFlag.CreateAlways to overwrite the file if 
                            it already exists.  Pass StreamFlag.LoopAtEof to
                            have the stream automatically return to the 
                            beginning of the file once the end is reached.
        ...
        streamID = QBYTE()
        result = QuickUsbLibrary.QuickUsbReadBulkDataStartStreamToFile(self.hDevice, ctypes.create_string_buffer(path), QULONG(numBuffers), QULONG(bufferSize), QULONG(maxTransfers), ctypes.byref(streamID), QULONG(flags))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
    ...
    def WriteBulkDataStartStreamFromFile(self, path, numBuffers, bufferSize, maxTransfers, flags):
        ...
        #QRETURN <= [QHANDLE, PQCHAR, QULONG, QULONG, QULONG, PQBYTE, QULONG]
        streamID = QBYTE()
        result = QuickUsbLibrary.QuickUsbWriteBulkDataStartStreamFromFile(self.hDevice, ctypes.create_string_buffer(path), QULONG(numBuffers), QULONG(bufferSize), QULONG(maxTransfers), ctypes.byref(streamID), QULONG(flags))
        if not result:
            self.lastError = self.GetLastError()
            self.lastDriverError = self.GetLastDriverError()
    ...
    def ProcessStream(self, streamID, milliseconds):
        ...
        Keyword Arguments:
            ...
            milliseconds -- The number of milliseconds the caller is willing 
                            ...
                            error Error.Timeout.  If milliseconds is zero, the
                            function will process any requests that are 
                            completed or timeout immediately.
...
if __name__ == '__main__':
    ...
    print("DLL Version: %d.%d.%d" % (major, minor, rev))
    print()
