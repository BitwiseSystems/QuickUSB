...
Public Function WriteDefault(ByVal address As Integer, ByVal Value As Integer) As Long
   Dim hDevice As Long
   Dim result As Long
...
Public Function ReadDefault(ByVal address As Integer, ByRef Value As Integer) As Long
   Dim hDevice As Long
   Dim result As Long
...
Public Function UploadFPGA(fileName As String, Optional status As StatusBar) As Boolean
   ...
   If (result = 0) Then
      MsgBox "Cannot start FPGA configuration"
      Close inputFile
      QuickUsbClose hDevice
      UploadFPGA = False
      Exit Function
